...
1. Open the package in development mode. Update the cache for all
   supported pkgdown versions. For example:
...
2. Run the minimization function to create a minimal cache in pkgdown.offline:

```r
pkgdown.offline:::minimize_cache(path_cache)
```

3. Now the package contains the minimized cache with deduplicated files in `inst/cache/`. Commit and push.
